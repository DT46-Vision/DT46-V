...
def generate_launch_description():
    ...
    # ==================== 2. 明确写死 Hero 专属的参数文件路径 ====================
    # 相机参数 (Hero)
    hero_camera_params = os.path.join(hik_camera_pkg, 'config', 'hero_camera_params.yaml')
    hero_camera_info   = 'package://hik_camera/config/hero_camera_info.yaml'

    # 算法参数 (Hero)
    hero_detector_params = os.path.join(rm_detector_pkg, 'config', 'detector_params_hero.yaml')
    hero_tracker_params  = os.path.join(rm_tracker_pkg, 'config', 'tracker_params_hero.yaml')
    ...
    return LaunchDescription([
        ...
        # ----------- 2. 启动海康相机 (强绑定 Hero 标定参数) -----------
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(hik_camera_pkg, 'launch', 'hik_camera.launch.py')
            ),
            launch_arguments={
                'params_file': hero_camera_params,
                'camera_info_url': hero_camera_info,
                'use_sensor_data_qos': 'false' # 根据你的需求开启或关闭
            }.items()
        ),

        # ----------- 3. 启动核心视觉算法 (强绑定 Hero 算法参数) -----------
        Node(
            package="rm_detector",
            ...
            output="screen",
            emulate_tty=True,
            parameters=[hero_detector_params],
        ),
        ...
        Node(
            ...
            output="screen",
            emulate_tty=True,
            parameters=[hero_tracker_params],
        ),
